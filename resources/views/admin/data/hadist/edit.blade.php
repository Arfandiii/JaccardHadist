    ...
    <main class="container mx-auto px-6 py-8">
        <div class="mb-6 flex items-center text-sm text-gray-900 space-x-2">
            <a href="{{ route('admin.dashboard.data') }}" class="text-blue-600 hover:underline font-medium">&larr;
                Kembali</a>
            <span>/</span>
            <h2 class="font-bold mb-2 text-center text-3xl">Edit Data Hadist</h2>
        </div>
        <div class="p-6 mb-10 mx-auto bg-white rounded-lg shadow-md">
            <div class="flex flex-wrap w-full mx-auto">
                <form action="{{ route('admin.hadist.update', $hadist->id) }}" method="POST"
                    ...
                    <div class="mb-2">
                        ...
                        <input type="number" name="no_hadist" id="no_hadist" required
                            ...
                            placeholder="Masukkan Nomor Hadist" value="{{ old('no_hadist', $hadist->no_hadist) }}">
                        @error('no_hadist')
                    ...
                    <div class="mb-2">
                        ...
                        <select name="kitab_id" id="kitab_id" required
                            ...
                            <option value="{{ $kitab->id }}" {{ old('kitab_id', $hadist->kitab_id)==$kitab->id ?
                                'selected' : '' }}>
                                {{ $kitab->nama_kitab }}
                            </option>
                            @endforeach
                    ...
                    <div class="mb-2">
                        ...
                        <select name="perawi_id" id="perawi_id" required
                            ...
                            <option value="{{ $perawi->id }}" {{ old('perawi_id', $hadist->perawi_id)==$perawi->id ?
                                'selected' : '' }}>
                                {{ $perawi->nama_perawi }}
                            </option>
                            @endforeach
                    ...
                    <div class="mb-2">
                        ...
                        <textarea name="isi_hadist" id="isi_hadist" rows="5" required
                            class="bg-gray-50 focus:outline-none focus:ring-1 border border-green-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5"
                            placeholder="Masukkan Isi Hadist">{{ old('isi_hadist', $hadist->isi_hadist) }}</textarea>
                        @error('isi_hadist')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
